...
our @forkArray = ();
our $errfile;
our ($resFile, $recFile, $incrFile, $dfCopyFile, $failedDf, $dfLck, $tmpMisc, $tstamp, $compFile);
our $dbId;
...
sub usage
{
   ...
   for cross-platform transportable tablespaces.
    ...
    usage: $0
     ...
     -L: Clear the error file
     --index: In the file rec.txt, the format is like for example
              4:::8,2,VB\$_4062672895_3116206I,1090834,3115959,1
              
              The index is before :::, it denotes the index of incremental 
              backup that was taken after the full backup   
EOF
   exit;
}
...
sub RunRMANCmd
{
   ...
   if ($errorSeen)
   {
      debug3 "$rman_cmd returned failure\n";
      @resArray = ();
   }
   else
   {
      debug3 "$rman_cmd returned success\n";
   }
   return @resArray;
}
...
sub checkEleRecDfnoExists
{
   ...
   {
      chomp ($x);
      debug4 "checkEleRecDfnoExists:: $x, $ele";
      if ($x =~ m/\#.*/)
      {
         next;
      }
...
EOF\n";
   ...
   if ($platId == 0)
   {
      Die("getPlatId: Unable to fetch platform id");
   }
   return $platId;
}
...
sub CreateResRecInpFile
{
   ...
   print "CreateResRecInpFile arun\n";
   $platId = getPlatId();
...
sub getVBName
{
   my ($iterCnt, $fno, $ckpScn, $platId, $bsKey, $resEnt, $filArrayP) = @_;
   my $sqlOutput ;
   my @resSysArray = ();
   my $platName = '';
   my $sqlplus_ra_cmd =
...
EOF\n";
   ...
   foreach my $x (@resSysArray)
   {
      if ($x =~ m/.*handle=(.*?)\s+ckp_scn=(.*)/)
      {
         ...
         my $fStr = "$iterCnt".":::$fno,$platId,$1,$2,$bsKey,1\n";
         push (@$filArrayP, $fStr);
      }
   }
   if ($resEnt == 0)
...
sub creUpdRecFile
{
   ...
   foreach my $x (@filArray)
   {
      ...
      if ($x =~ m/(.*):::(.*?),(.*?),(.*?),(.*?),(.*?),(.*)/)
      {
         ...
         getVBName($iterCnt, $fno, $ckpScn, $platid, $bsKey, $x, \@updFileArray);
...
sub updateRecoverDFCopy
{
   ...
   #For restore update the datafilecopy
   if ($context->{"restore"})
   {
...
sub createRecCmd
{
   my ($dfFailedArrayP, $dfSucArrayP) = @_;
   my @dfFailedArray = @$dfFailedArrayP;
   my @dfSucArray = @$dfSucArrayP;
   my $file = $recFile;
   my @inFilArray = ();
   my $platName = '';
   my $trialCnt = 3;
   ...
   foreach my $x (@inFilArray)
   {
      ...
      if ($x =~ m/(.*):::(.*?),(.*?),(.*?),(.*?),(.*?),(.*)/)
      {
         ...
         if (scalar(@dfFailedArray) > 0)
         {
            ...
            {
               debug3Message("File $fno already recovered");
               next;
            }
         }

         if (scalar(@dfSucArray) > 0)
         {
            if (checkDfComp(\@dfSucArray, $x))
            {
               debug3Message("File $fno already recovered");
               next;
            }
      ...
      if ($rollParallel)
      {
         ...
         if ($pid == 0)
         {
            ...
            while ($count < $trialCnt)
            {
               ...
               if ($retCount == 0)
               {
                  debug3Message ("Recover $fno failed");
               }
               else
               {
                  debug3Message ("Recover $fno succeded");
               }
               if ($retCount != 0)
               {
                  updateRecoverDFCopy($x, $fno, \@resSysArray);
                  exit (0);
               }
               $count = $count + 1;
            }
...
sub createResCmd
{
   ...
   foreach my $x (@inFilArray)
   {
      ...
      $x = Trim($x);
      debug5("createResCmd:$x");
      if ($x =~ m/.*:::(.*?),(.*?),(.*?),(.*?),(.*)/)
      {
         print "arun here createResCmd\n";
         $platid = $2;
         $fno = $1;
         $bkpSetName = $3;
         $ckpScn = $4;
         ...
         {
            printMessage ("File $fno already restored");
            next;
         }
      ...
      if ($rollParallel)
      {
         ...
         if ($pid == 0)
         {
            ...
            while ($count < 10)
            {
               ...
               if ($retCount == 0)
               {
                  debug3Message ("Restore $fno failed");
               }
               else
               {
                  debug3Message ("Restore $fno succeded");
               }
               if ($retCount != 0)
               {
                  updateRecoverDFCopy($x, $fno, \@resSysArray);
...
sub performRecover
{
   my (@dfCopyArray, @dfFailedArray, @dfSucArray, @inFilArray);

   open FILE, "$recFile" or Die ("createRecCmd:Unable to read file $recFile");
   @inFilArray = <FILE>;
   close FILE;

   @dfFailedArray = readFile($failedDf, 1, 0);
   @dfSucArray = readFile($compFile, 1, 1);

   #Check if sufficient backups have been applied
   my $sucCount = scalar(@dfSucArray);
   if ($sucCount > 0)
   {
      my ($lastApplied, $lastCFile, $levelToApply, $levelReq);
      $lastApplied = $dfSucArray[-1];
      if ($lastApplied =~ m/(.*):::.*/)
      {
         $lastApplied = $1;
         if ($lastApplied =~ m/\#(.*)/)
      ...
      debug5 "lastcfile=$lastCFile, lastApplied=$lastApplied";
      $levelToApply = $lastApplied + 1;
      if (defined($context->{"autoindex"}))
      {
         if ($levelToApply > $lastCFile)
         {
            printMessage ("No incremental index $levelToApply found, last available is $lastCFile");
            exit(0);
         }
         $context->{"index"} = $levelToApply;
      }
      my $levelApplied = $context->{"index"};
      while ($levelApplied <= $lastCFile)
      {
         @dfFailedArray = readFile($failedDf, 1, 0);
         @dfSucArray = readFile($compFile, 1, 1);
         $levelApplied = $levelApplied + 1;
         checkErrFile();
         createRecCmd(\@dfFailedArray, \@dfSucArray);
         $context->{"index"} = $levelApplied;
      }
   }
   else
   {
